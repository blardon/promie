...
for annotation_entry in frame_annotations:
    ...
    ws_for_annotation = annotation_latents[annotation]
    ...
    if final_ws is None:
        final_ws = chromagram_weighted_ws
    else:
        final_ws = torch.cat((final_ws, chromagram_weighted_ws), 0)
print(f"Final ws: {final_ws.size()}")
...
for i, annotation_entry in enumerate(frame_annotations):
    ...
    ws_to_fade = final_ws[center_frame-fade_frame_count//2:center_frame+fade_frame_count//2]
    ws_to_fade = audio_processing.gaussian_filter(ws_to_fade, 2)
    final_ws[center_frame-fade_frame_count//2:center_frame+fade_frame_count//2] = ws_to_fade
...
#######################################
for i_frame, dlatent in enumerate(final_ws):
